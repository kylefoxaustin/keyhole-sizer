...
|---|---|---|
| **v2.0.1** | 2026-07-26 | **Vision Amendment 5 restored, and moved into the engine.** v1.1.1's CNN/vision anchor BW-scaling had been written into `app.py`; v2.0.0 replaced `app.py` wholesale and the fix went with it, surviving only in the never-imported `app_vertical_legacy.py`. For 46 days the engine carried stock vision anchors **verbatim** onto memory-upgrade clones — flat fps against up to 2.19× bandwidth — and badged them 🟢 `measured` on parts that were never built, while the LLM path on the same clone object correctly reported 🟡. **Severity: LATENT, not user-visible** — the memory-upgrade control is gated to Mid/High (`app.py`, the sole `hw_with_memory` call site) and Mid/High carry no `measured_vision_overrides`, so the affected combinations were reachable through the engine API but not through the UI. Fixed on the merits (the provenance bug is real) rather than as a user-facing regression. Now fixed in `sizer/npu_model.py` (`_anchor_bw_scale()` at the `project_vision` anchor lookup) where a surface rewrite can't reach it, plus the badge degrades 🟢 → 🟡 `same_class_anchor` on `bw_projected` clones, matching `project_llm`. Invariant `fps_ratio == bw_ratio` now holds exactly across all 15 vision tier×pipeline×resolution×upgrade cells; stock tiers and the whole LLM path unchanged. Also refreshes `CLAUDE.md` + `PHASE3_PARITY_REPORT.md`, whose "deferred follow-up" notes had been wrong for 60 days. |
| **v2.0.0** | 2026-06-10 | **Horizontal-layout go-live.** `app.py` is now the horizontal app: top control strip (tier pills + popovers + workload pills + ⚙ settings), no sidebar, full-width results, on-screen KPIs, per-workload scoped detail tabs + KPI sections + uber XLSX export. Promoted from `app_horizontal_prototype.py`; shared-password gate ported in. Previous vertical layout preserved at `app_vertical_legacy.py` (not imported). Rollback anchor: tag v1.2.0. |
| **v1.2.0** | 2026-06-07 | **VLA workload arc + FP4 precision.** 5-model VLA catalog across 3 topologies with dual-loop projection, multi-camera + fleet scaling, operating-rate sliders (`vlm_hz` / `action_hz`), 3-state confidence badge and FP-required hard gate; NVFP4 dtype recognition + the NPU precision-set selector (INT8 / INT+FP8 / INT+FP8+FP4) via ratchet v0.2.6. Horizontal layout still a prototype at this tag — HEAD `app.py` is the original vertical layout, making this the rollback anchor for the v2.0.0 go-live. |
| **v1.1.1** | 2026-05-27 | Amendment 5 mirrored onto the CNN/vision overlay (`_maybe_anchor_overlay_cnn`) — the deferred follow-up from v1.1.0. Pre-fix the helper dropped the CNN anchor on any memory-upgrade clone, producing a measured→projection discontinuity (first upgrade tier could read lower fps than stock). Post-fix the helper BW-scales the measured anchor by `mem_bandwidth_gbs / stock_mem_bandwidth_gbs` for `bw_projected` clones; ms scales inversely, fps scales directly. Stock tiers unchanged. `app.py` only (18 insertions, 4 deletions); auto-reload-safe on Streamlit Cloud. ⚠ **Superseded — this fix was silently un-shipped by v2.0.0** (which replaced `app.py`, the only file it touched) and was not actually live again until v2.0.1. |
