...
|---|---|---|
| **v2.0.1** | 2026-07-26 | **Vision Amendment 5 restored, and moved into the engine.** v1.1.1's CNN/vision anchor BW-scaling had been written into `app.py`; v2.0.0 replaced `app.py` wholesale and the fix went with it, surviving only in the never-imported `app_vertical_legacy.py`. For 46 days the shipped app carried stock vision anchors **verbatim** onto memory-upgrade clones — flat fps against up to 2.19× bandwidth, understated by up to 54% — and badged them 🟢 `measured` on parts that were never built, while the LLM path on the same clone object correctly reported 🟡. Now fixed in `sizer/npu_model.py` (`_anchor_bw_scale()` at the `project_vision` anchor lookup) where a surface rewrite can't reach it, plus the badge degrades 🟢 → 🟡 `same_class_anchor` on `bw_projected` clones, matching `project_llm`. Invariant `fps_ratio == bw_ratio` now holds exactly across all 15 vision tier×pipeline×resolution×upgrade cells; stock tiers and the whole LLM path unchanged. Also refreshes `CLAUDE.md` + `PHASE3_PARITY_REPORT.md`, whose "deferred follow-up" notes had been wrong for 60 days. |
